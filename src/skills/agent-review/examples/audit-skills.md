...
- Required frontmatter (name, description; verb/domain for review leaves) — A.1
...
- No `Skill()` calls from leaves (only the audit dispatcher may invoke) — E.1 (R1)
- No hardcoded `bun test.ts` in review-flavor skills — F.1 (R4)
...
## nit
src/skills/agent-review/ — agent/skills.md#D.2 — [sub_surface: skills]
  No examples/ directory; description mentions /agent-review slash-command. [tag: examples]
