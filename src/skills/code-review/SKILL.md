...
- Fix shapes: consolidation onto a canonical helper, propagation from a reference file to peers, structural restructure (file moves with the importer-update matrix), and security remediation (parameterise SQL, swap weak crypto, route secrets through env, add ownership checks). Each is applied per the rule cite in step 4. The rules file describes each.
- Slop removal (defensive code, restating comments, backwards-compat shims, scope creep, impossible-case throws, `as any` casts) and tactical simplification live in `code-suggest` (Flow A). Route there for those.
