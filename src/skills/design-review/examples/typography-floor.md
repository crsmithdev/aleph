...
This case is the simplest of the three — pure token discipline. But it's the conform pass that runs *most often* in practice, because every new component imported or pasted from another project tends to bring its own ad-hoc font sizes. Run `/design-review --mode fix` against this case after every "add a new page from a template" task, and the codebase stays inside the type system instead of drifting into one-off `text-[Npx]` values. The floor itself is small (a handful of `text-[10px]` hits today), but the *discipline* is high-leverage — every drop below 12px gets caught here.
