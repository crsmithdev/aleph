...
Read `~/.construct/handoffs/current.md` in full. If the file does not exist, say so and stop — do not guess.
...
2. Archive the handoff so it isn't picked up twice: `mv ~/.construct/handoffs/current.md ~/.construct/handoffs/$(date +%Y-%m-%d-%H%M%S).md`.
