...
When the user provides a feature name or description (or none — ask), invoke `git` (Phase 1: Isolate) to set up an isolated worktree:
...
When implementation is complete and tests pass, invoke `git` again (Phase 2: Land) to handle merge, PR, or cleanup:
...
3. Run `git` (Phase 2) after each one completes
