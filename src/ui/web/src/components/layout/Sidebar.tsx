...
function SidebarLink({ to, label, icon, depth = 0, collapsed = false, disabled = false, isGroupHeader = false }: {
  ...
  if (disabled) {
    return (
      <div
        className={clsx(
          'flex items-center gap-2.5 py-1.5 text-[13px] font-sans cursor-not-allowed',
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        clsx(
          'flex items-center gap-2.5 py-1.5 text-[13px] font-sans transition-colors rounded',
          depth > 0 ? 'pl-5 pr-2' : 'pl-2 pr-2',
          isActive
            ? isGroupHeader
...
function BrandMark() {
  return (
    <div
      className="grid place-items-center w-7 h-7 rounded-md bg-bg-tertiary font-heading font-black text-accent text-sm shrink-0"
      ...
      <div className={clsx(
        ...
        {collapsed ? (
          ...
          <NavLink to="/" className="flex items-center gap-2.5 min-w-0">
            <BrandMark />
            <span className="font-heading text-[15px] font-bold text-text-primary truncate">construct</span>
          </NavLink>
        )}
        <button
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3">
        {!collapsed && navGroups.map((group, gi) => (
          <div key={gi} className={gi > 0 ? 'mt-2' : ''}>
            <div className="font-mono text-[10px] uppercase tracking-[0.08em] text-text-muted px-2 pt-3 pb-1">
              {group.label}
            </div>
