/**
 * Unified dev-server logger.
 *
 * One entry point — `log({source, level, msg, ...fields})` — fans events out to:
 *   1. NDJSON file at ~/.construct/logs/dev-YYYY-MM-DD.ndjson (rotated daily, append-only)
 *   2. Pretty colorized stdout in a vite-style compact format: `HH:MM:SS [source] msg`
 *
 * Sources used in this codebase: dev, api, supervisor, worker, vite, fastify, ddl, research.
 * `worker_id` is rendered inline as `[worker @ N]` per the project format spec.
 *
 * Intentionally tiny — no chalk/picocolors dep. Colors disabled when stdout is not a TTY
 * (so piped log files are clean ANSI-free) and when NO_COLOR is set.
 */
 ...
 * Per-request lines come from the explicit onResponse hook in app.ts and bypass
...
export function createViteLogger(): ViteLogger {
  ...
  function strip(msg: string): string {
    // Vite injects its own ANSI + leading "HH:MM:SS [vite]" — we own formatting now.
