use {
    crate::{
        ...
        material::Material,
        model::{MeshDesc, PrimitiveDesc, SamplerDesc, TextureDesc, Vertex},
        util, MaterialHandle, MeshHandle, TextureHandle,
    },
    aleph_vk::{Extent2D, ImageUsageFlags},
    anyhow::{bail, Result},
    ash::vk,
...
pub fn load_scene(path: &str, mut assets: &mut Assets) -> Result<SceneDesc> {
    ...
    for index in 0..document.textures().len() {
        ...
        let sampler = load_sampler(info.sampler());
...
fn load_mesh(
    ...
    for (i, primitive) in source.primitives().enumerate() {
        ...
        log::debug!("  Primitive {i} -> {n_vertices} vertices");
    }
