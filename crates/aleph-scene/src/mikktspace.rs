...
fn get_tex_coord<I: MikktGeometry>(geometry: &mut I, index: usize) -> Vec3 {
    ...
    let val = Vec3::new(tex_coord.x, tex_coord.y, 0.0);
    val
}
...
fn index_to_face_vert(index: usize) -> (usize, usize) { (index >> 2, index & 0x3) }

fn face_vert_to_index(face: usize, vert: usize) -> usize { face << 2 | vert & 0x3 }
...
unsafe fn Normalize(v: Vec3) -> Vec3 { return (1.0 / v.length()) * v; }

unsafe fn VNotZero(v: Vec3) -> bool { NotZero(v.x) || NotZero(v.y) || NotZero(v.z) }

unsafe fn NotZero(fX: f32) -> bool { fX.abs() > 1.17549435e-38f32 }
...
mod test {
    ...
    impl MikktGeometry for Context {
        fn num_faces(&self) -> usize { self.mesh.faces.len() }
    ...
    impl ControlPoint {
        fn new(uv: [f32; 2], dir: [f32; 3]) -> Self { Self { uv, dir } }
    }
    ...
    fn cube_tangents_should_equal_reference_values() {
        ...
        for i in 0..context.results.len() {
            let result = &context.results[i];
            let expected = &expected_results[i];
            assert_eq!(result.tangent, expected.tangent);
            assert_eq!(result.bi_tangent, expected.bi_tangent);
            assert_eq!(result.mag_s, expected.mag_s);
            assert_eq!(result.mag_t, expected.mag_t);
            assert_eq!(
                result.bi_tangent_preserves_orientation,
                expected.bi_tangent_preserves_orientation
            );
            assert_eq!(result.face, expected.face);
            assert_eq!(result.vert, expected.vert);
        }
    }
}
