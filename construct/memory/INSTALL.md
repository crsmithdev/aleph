...
- Hook files exist: `session-start.ts`, `rating-capture.ts`, `session-summary.ts`, `memory-gate.ts` in `construct/memory/hooks/`
...
- `memory/hooks/memory-gate.ts` registered under `Stop` in `settings.json`
