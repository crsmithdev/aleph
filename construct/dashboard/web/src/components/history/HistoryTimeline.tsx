import type { HistoryLog, HistoryEvent } from '@goal-tracker/shared';

const eventMeta: Record<HistoryEvent, { label: string; color: string; dot: string }> = {
  goal_created:    { label: 'Goal created',      color: 'text-blue-400',    dot: 'bg-blue-500' },
  goal_updated:    { label: 'Goal updated',       color: 'text-gray-400',    dot: 'bg-gray-500' },
  ...
  category_added:  { label: 'Category added',     color: 'text-green-400',   dot: 'bg-green-500' },
  category_removed:{ label: 'Category removed',   color: 'text-red-400',     dot: 'bg-red-500' },
  ...
  note_deleted:    { label: 'Note deleted',       color: 'text-red-400',     dot: 'bg-red-500' },
  todo_linked:     { label: 'Todo linked',        color: 'text-indigo-400',  dot: 'bg-indigo-500' },
  todo_unlinked:   { label: 'Todo unlinked',      color: 'text-gray-400',    dot: 'bg-gray-500' },
  archived:        { label: 'Archived',           color: 'text-gray-400',    dot: 'bg-gray-600' },
  unarchived:      { label: 'Unarchived',         color: 'text-gray-300',    dot: 'bg-gray-400' },
};
...
export function HistoryTimeline({ entries }: HistoryTimelineProps) {
  if (entries.length === 0) {
    return (
      <p className="text-sm text-gray-600 py-4">No history yet.</p>
    );
  }
  ...
  return (
    <ol className="relative border-l border-gray-800 ml-2 flex flex-col gap-4">
      {sorted.map((entry) => {
        ...
        return (
          <li key={entry.id} className="ml-4">
            <span
              className={`absolute -left-1.5 w-3 h-3 rounded-full border-2 border-gray-950 ${meta.dot}`}
            />
            <div className="flex items-baseline gap-2 flex-wrap">
              ...
              <span className="text-xs text-gray-700 ml-auto">{formatTime(entry.createdAt)}</span>
            </div>
          </li>
        );
      })}
