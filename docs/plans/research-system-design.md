...
| Current plan | Same mapping; plan is an internal struct created at low envelope spend, re-planned at milestone checkpoints |
...
**For the system.** Keep deterministic table-driven planning as the primary path; it's the only approach in the survey that produces reproducible runs, and reproducibility matters more for a tool used heavily by one operator than for a benchmark-chasing research artifact. Make the topic taxonomy escapable: when a query doesn't fit the canonical clusters, the system should know it doesn't fit rather than route to a "miscellaneous" bucket. Promote the plan to a first-class, persisted artifact so it's diffable and editable. The plan should be regenerable from the inputs (question, shape, topic, envelope) so re-planning is a fresh table lookup, not a mutation of internal state.
...
The deterministic routing has a property no comparator achieves: reproducibility. Same query → same plan, every time. That's valuable for diagnostics, debugging, and longitudinal evaluation.

**For the system.** Keep the typed-shape detection and role priming — both are genuine differentiators and both contribute to reproducibility. Make the topic taxonomy substantially richer: either grow to 20+ clusters, or let the inferring LLM produce a free-text cluster label with the canonical clusters as preferred labels when they apply. Detect URL-grounded queries (when the prompt contains a URL, fetch it and use its contents as the seed source set rather than running canon enumeration on adjacent topics). Make canon enumeration shape-conditional: for comparison queries on a user-provided source list, the list *is* the canon — don't enumerate.

Add output shape as a populated, inferred, editable field, distinct from question shape. Question shape governs how to investigate; output shape governs what to produce.
