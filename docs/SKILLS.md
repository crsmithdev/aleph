...
| `design-audit` | Systematic UI/UX design audit against 15 dimensions: hierarchy, spacing, typography, color, alignment, components, motion | `design audit`, `audit the design`, `audit the ui`, `audit ui`, `design review`, `make this feel professional`, `make this look professional`, `polish the interface`, `ux audit`, `visual hierarchy`, `spacing audit`, `design consistency`, `ui architecture`, `reduce clutter` |
| `design-standards` | Audit UI code against web interface best practices: accessibility, forms, performance, navigation, content handling | `web standards`, `check accessibility`, `accessibility audit`, `a11y`, `semantic html`, `web guidelines`, `best practices`, `review ux`, `audit for a11y`, `check web standards`, `form validation`, `performance audit` |
| `design-type` | Professional typography rules for UI — proper quote marks, dashes, spacing, letterspacing, line length | `typography`, `fix typography`, `typographic`, `curly quotes`, `em dash`, `en dash`, `letterspacing`, `line length`, `font pairing`, `text hierarchy`, `review layout`, `html entities`, `opentype features`, `tabular nums` |
